...
Codex summaries reuse the locally installed Codex CLI and its saved sign-in. Install Codex and run `codex login` once before using desktop summaries. Summary requests use current web search, so they require internet access and count against the signed-in Codex account's usage.
...
The Last Hour has no account system, tracking, advertising, telemetry, or application server. Feed requests go to Google News, and opening a story sends the user to the linked publisher. Cached feed data stays in the application's private storage on the device. When a desktop user requests a Codex summary, story metadata and the URL are sent through that computer's signed-in Codex CLI. The desktop weather widget requests current Huntsville conditions from [Open-Meteo](https://open-meteo.com/) without an API key or device location. Android AI summaries are optional; when enabled, the selected article text and summary prompt are sent directly to Google's Gemini API using the API key supplied by that device's user. The encrypted API key is stored in private app storage with backup disabled.
